...
--Begin Fun.lua By @tgMember
--Special Thx To @sajjad_021
--------------------------------
			...
			function tophoto_cb(arg,data)
				if data.content_.sticker_ then
					...
					if file_exi(tostring(name), tostring(apath), tostring(pasvand)) then
						...
						tdcli.sendPhoto(msg.to.id, 0, 0, 1, nil, pfile, "@tgMember", dl_cb, nil)
					else
			...
			function tosticker_cb(arg,data)
				if data.content_.ID == 'MessagePhoto' then
					...
					if file_exi(file..'_(1).jpg', tcpath..'/data/photo', 'jpg') then
						...
						tdcli.sendDocument(msg.chat_id_, 0, 0, 1, nil, pfile, '@tgMember', dl_cb, nil)
					else
...
if matches[1] == 'voice' then
 local text = matches[2]
  ...
  local file = download_to_file(url,'tg-Guard.mp3')
 				tdcli.sendDocument(msg.to.id, 0, 0, 1, nil, file, '@tgMember', dl_cb, nil)
   end
end
...
--------------------------------
	if matches[1]:lower() == "photo" then 
		...
		tdcli.sendPhoto(msg.to.id, 0, 0, 1, nil, file, "@tgMember", dl_cb, nil)
	end
...
if not lang then
helpfun = [[
_TeleGram Guard Fun Help Commands:_
...
helpfun = [[
_راهنمای فان ربات تلگرام گارد:_
